...
export default function LedgerAdjustmentForm({ customerId, onSuccess }) {
    ...
    try {
      const res = await fetch(`/api/ledger/customer/${customerId}/entry`, {
        ...
        body: JSON.stringify({
          ...
          notes,
          createdBy: 1,
          entryType: side === 'debit' ? 'adjustment_debit' : 'adjustment_credit',
          description:
            description ||
            (side === 'debit' ? 'Manual debit adjustment' : 'Manual credit adjustment'),
        }),
      })
  ...
  return (
    <div className="bg-white border rounded-xl p-4 shadow-sm">
      <h3 className="text-lg font-semibold mb-4">Ledger Adjustment</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div>
          ...
          >
            <option value="debit">Debit - Customer se lena hai</option>
            <option value="credit">Credit - Customer ne diya</option>
          </select>
        </div>
