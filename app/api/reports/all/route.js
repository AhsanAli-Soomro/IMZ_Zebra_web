...
export async function GET(request) {
  try {
    ...
    const costColumn =
      'Purchase unit cost (stock ID, then item-name, then stock purchase rate)'
    ...
    const itemProfit = await db.query(
      ...
      purchase_cost_by_name AS (
       ...
       SELECT
        ...
        COALESCE(SUM(COALESCE(NULLIF(sii.amount, 0), sii.qty * CASE WHEN COALESCE(sii.weight, 0) > 0 THEN sii.weight ELSE 1 END * sii.price)), 0) - COALESCE(SUM((CASE WHEN sii.price > 0 THEN COALESCE(NULLIF(sii.amount, 0), sii.qty * CASE WHEN COALESCE(sii.weight, 0) > 0 THEN sii.weight ELSE 1 END * sii.price) / sii.price ELSE sii.qty * CASE WHEN COALESCE(sii.weight, 0) > 0 THEN sii.weight ELSE 1 END END) * COALESCE(
          NULLIF(pc_stock.avg_purchase_price, 0),
          NULLIF(pc_name.avg_purchase_price, 0),
          NULLIF(s.purchase_rate, 0),
          NULLIF(s.purchase_price, 0),
