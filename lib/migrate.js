...
export function runMigrations(dbPathFromElectron, schemaPathFromElectron) {
  ...
  try {
    ...
    // Keep exactly one ledger account per customer/supplier. Older databases did
    // not have a unique constraint, so the startup backfill created duplicates.
    // Move every historical entry to the oldest account before removing extras.
    if (tableExists(db, 'ledger_accounts') && tableExists(db, 'ledger_entries')) {
      db.transaction(() => {
        db.prepare(`
          UPDATE ledger_entries
          SET account_id = (
            SELECT MIN(canonical.id)
            FROM ledger_accounts current
            INNER JOIN ledger_accounts canonical
              ON canonical.owner_type = current.owner_type
             AND canonical.owner_id = current.owner_id
            WHERE current.id = ledger_entries.account_id
          )
          WHERE account_id IN (
            SELECT duplicate.id
            FROM ledger_accounts duplicate
    ...
    // INDEXES
    // =========================
    createIndexIfNotExists(db, `CREATE UNIQUE INDEX IF NOT EXISTS idx_ledger_accounts_owner_unique ON ledger_accounts(owner_type, owner_id);`)
    ...
    createIndexIfNotExists(db, `CREATE INDEX IF NOT EXISTS idx_bills_date_deleted ON bills(invoice_date, deleted_at);`)
    createIndexIfNotExists(db, `CREATE INDEX IF NOT EXISTS idx_bills_customer_date ON bills(customer_id, invoice_date);`)
    createIndexIfNotExists(db, `CREATE INDEX IF NOT EXISTS idx_purchase_invoices_date_deleted ON purchase_invoices(invoice_date, deleted_at);`)
    createIndexIfNotExists(db, `CREATE INDEX IF NOT EXISTS idx_purchase_invoices_supplier_date ON purchase_invoices(supplier_id, invoice_date);`)
    createIndexIfNotExists(db, `CREATE INDEX IF NOT EXISTS idx_ledger_entries_account_date_deleted ON ledger_entries(account_id, entry_date, deleted_at);`)
    createIndexIfNotExists(db, `CREATE INDEX IF NOT EXISTS idx_ledger_entries_date_deleted ON ledger_entries(entry_date, deleted_at);`)
    createIndexIfNotExists(db, `CREATE INDEX IF NOT EXISTS idx_cash_transactions_date_type_deleted ON cash_transactions(tx_date, tx_type, deleted_at);`)
    createIndexIfNotExists(db, `CREATE INDEX IF NOT EXISTS idx_stock_movements_stock_type ON stock_movements(stock_id, movement_type);`)
    createIndexIfNotExists(db, `CREATE INDEX IF NOT EXISTS idx_sales_invoice_items_stock_bill ON sales_invoice_items(stock_id, bill_id);`)
    createIndexIfNotExists(db, `CREATE INDEX IF NOT EXISTS idx_purchase_invoice_items_stock_invoice ON purchase_invoice_items(stock_id, purchase_invoice_id);`)
    createIndexIfNotExists(db, `CREATE INDEX IF NOT EXISTS idx_customers_deleted_id ON customers(deleted_at, id);`)
    createIndexIfNotExists(db, `CREATE INDEX IF NOT EXISTS idx_suppliers_deleted_id ON suppliers(deleted_at, id);`)
    createIndexIfNotExists(db, `CREATE INDEX IF NOT EXISTS idx_stocks_category_deleted ON stocks(category, deleted_at);`)
    ...
    if (tableExists(db, 'customers') && tableExists(db, 'ledger_accounts')) {
      const customers = db.prepare(`SELECT id, name FROM customers WHERE deleted_at IS NULL OR deleted_at = ''`).all()
    ...
    if (tableExists(db, 'suppliers') && tableExists(db, 'ledger_accounts')) {
      const suppliers = db.prepare(`SELECT id, name FROM suppliers WHERE deleted_at IS NULL OR deleted_at = ''`).all()
      const stmt = db.prepare(`
        INSERT OR IGNORE INTO ledger_accounts (owner_type, owner_id, account_name)
        VALUES ('supplier', ?, ?)
      `)
