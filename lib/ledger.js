...
async function receiveCustomerPayment({
  ...
  const tx = sqlite.transaction(() => {
    ...
    if (bankAccountId) {
      ...
      const bankBalance = toNumber(bankAccount.current_balance) + safeAmount
      sqlite.prepare(`
        UPDATE bank_accounts
        SET current_balance = ?, updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
      `).run(bankBalance, bankAccountId)
...
async function paySupplier({
  ...
  const tx = sqlite.transaction(() => {
    ...
    if (bankAccountId) {
      ...
      if (!bankAccount) throw new Error('Selected bank account not found')
      if (toNumber(bankAccount.current_balance) < safeAmount) {
        throw new Error('The selected bank account has insufficient balance.')
      }
      ...
      sqlite.prepare(`
        INSERT INTO bank_transactions (
          ...
          reference_type, reference_id, description, notes, created_by,
          created_at, updated_at
        ) VALUES (?, ?, 'withdrawal', ?, ?, 'ledger_entry', ?, ?, ?, ?,
          CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
      `).run(
        bankAccountId,
        entryDate || todayDate(),
...
async function getCustomerLedger(customerId) {
  ...
  const entries = await db.query(
    `SELECT
      ...
      created_at,
      updated_at,
      (SELECT ct.payment_method FROM cash_transactions ct
       WHERE ct.reference_type = 'ledger_entry' AND ct.reference_id = ledger_entries.id
         AND ct.deleted_at IS NULL ORDER BY ct.id DESC LIMIT 1) AS payment_method,
      (SELECT ct.source_of_payment FROM cash_transactions ct
       WHERE ct.reference_type = 'ledger_entry' AND ct.reference_id = ledger_entries.id
         AND ct.deleted_at IS NULL ORDER BY ct.id DESC LIMIT 1) AS source_of_payment
     FROM ledger_entries
     WHERE account_id = ? AND deleted_at IS NULL
  ...
  return {
    ...
    entries: entries.reverse().map((row) => {
